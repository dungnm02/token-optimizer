...
sequenceDiagram
    ...
    Note over M,X: ❌ Polling qua trung gian model — ngữ cảnh đầy đủ mỗi lần poll
    loop mỗi 30 giây × 10
        M->>X: check_status()
        X-->>M: "đang chạy…" (ngữ cảnh bị tính phí lại mỗi lần)
    end
    end
    ...
    M->>M: tiếp tục với kết quả trong ngữ cảnh
    end
```

Một vòng poll 10 lần trên ngữ cảnh 100K token đốt ~1M token input để chẳng
...
- Chi phí chờ đợi giảm từ **O(số lần poll × kích thước ngữ cảnh)** xuống
