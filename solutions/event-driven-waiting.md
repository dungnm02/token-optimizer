...
**Ý tưởng:** Đừng bao giờ tốn một lượt gọi model chỉ để biết "chưa xong".
Hãy đưa việc chờ đợi ra *bên ngoài* model — dùng webhook, luồng sự kiện,
đánh thức theo lịch, hoặc workflow bền vững — sao cho model chỉ cần được
gọi đúng hai lần cho mỗi lần chờ: một lần để bắt đầu công việc, một lần
...
1. **Kết thúc lượt khi bị chặn bởi thế giới bên ngoài.** Agent nên coi
   "chờ đợi" là một trạng thái kết thúc cho lần gọi hiện tại, chứ không
   phải là khoảng thời gian cần lấp đầy bằng các lần kiểm tra liên tục.
...
4. **Khớp tần suất kiểm tra với tiến trình thực tế** nếu model phải tự lên
   lịch (các harness có timer đánh thức): một lần kiểm tra đúng vào thời
   điểm dự kiến hoàn thành sẽ tốt hơn N lần kiểm tra cách nhau từng khoảng
   ngắn — một lượt CI chạy ~8 phút xứng đáng với một lần đánh thức ~8
   phút, không phải mười sáu lần cách nhau 30 giây.
...
- Các agent đánh thức-theo-sự-kiện phải xây lại trạng thái làm việc khi
  khôi phục — nên kết hợp với caching (`prompt-caching.md`, TTL dài) để
  request khôi phục đọc lại lịch sử với chi phí rẻ, thay vì phải xử lý lại
  từ đầu.
