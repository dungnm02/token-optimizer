...
mà các model hiện tại không còn cần nữa. Hãy rà soát scaffolding này dựa
trên đánh giá (eval), xóa bỏ những gì model hiện nay đã tự làm mà không
cần yêu cầu, và giữ lại phiên bản prompt *được đo là* rẻ nhất ở cùng một
mức chất lượng.
...
Scaffolding tốn kém hai lần: vừa là **chi phí prompt cố định** trên mỗi
request, vừa là **output bị gây ra thêm**. Chẳng hạn, các chỉ dẫn như "tóm
tắt tiến độ sau mỗi 3 lệnh gọi tool" hay "giải thích lý luận của bạn từng
bước" khiến model sinh thêm token ở mỗi lượt — những token mà một model
hiện tại vốn dĩ sẽ không tự tạo ra (model reasoning đã tự suy luận ở bên
trong; các model agent hiện đại cũng đã tường thuật phù hợp sẵn). Ngoài
ra, danh sách bước quá quy định còn có thể *làm giảm* chất lượng output
trên các model mới hơn, vì các model này hoạt động tốt hơn khi chỉ được
cho mục tiêu + ràng buộc, thay vì một quy trình liệt kê chi tiết.
...
vụ hoàn thành** (prompt + output bị gây ra). Hãy triển khai (ship) biến thể
chiến thắng theo tiêu chí Pareto.

### 3. Tự động hóa tìm kiếm khi khối lượng đủ lớn để đáng công sức

Các framework tối ưu prompt sẽ tự động khám phá các cấu hình
chỉ-dẫn/few-shot theo đúng chỉ số (metric) của bạn. Các trình tối ưu hiện
đại thường xuyên tìm ra những prompt *ngắn hơn* nhưng đạt điểm cao hơn, vì
chúng chỉ giữ lại các chỉ dẫn thực sự chịu lực và bộ demo hiệu quả tối
thiểu.

### 4. Rà soát lại trên mỗi lần di chuyển model

Mỗi thế hệ model khiến một lớp scaffolding khác trở nên lỗi thời (các
hướng dẫn di chuyển liệt kê rõ lớp nào). Hãy đưa một bước rà soát loại bỏ
scaffolding vào checklist di chuyển model — vì prompt từng được tinh chỉnh
cho model cũ giờ đã vừa quá cồng kềnh, vừa tinh chỉnh sai hướng.

### 5. Chuyển chỉ dẫn hiếm khi cần vào context theo yêu cầu

Các đoạn tĩnh kiểu "cách xử lý tình huống X" mà chỉ áp dụng cho 2% request
thì nên đặt trong các tài nguyên tiết lộ dần dần (progressive disclosure)
— skill/playbook/file mà model chỉ tải khi liên quan — thay vì nhét vào
system prompt của mọi request. Mức tiết kiệm lớn
...
- Việc này đòi hỏi một bộ đánh giá (eval) thực sự — xóa bỏ mà không đo
  lường chính là cách các hồi quy chất lượng lọt qua. (Nếu bạn chưa có bộ
  đánh giá, xây dựng nó là điều kiện tiên quyết, và giá trị của nó sẽ tự
  bù đắp qua mọi giải pháp khác trong thư mục này.)
- Một số scaffolding tuy nhìn chung đã lỗi thời nhưng vẫn chịu lực cho các
  trường hợp biên *của bạn*; loại bỏ từng phần (ablation) sẽ phát hiện ra
  điều này, còn xóa hàng loạt thì không.
...
- Tiết kiệm output bị gây ra thường lớn hơn tiết kiệm prompt: bỏ các nghi
  thức tường thuật/CoT bắt buộc sẽ cắt giảm đáng kể lượng output mỗi lượt
  trên các route agent.
- Chất lượng thường *được cải thiện* như một hiệu ứng phụ: ít kích hoạt
  quá mức hơn, ít văn bản nghi thức hơn, và tuân theo chỉ dẫn tốt hơn trên
  các model hiện đại vốn hiểu theo nghĩa đen — các hướng dẫn di chuyển
  cũng ghi nhận rõ xu hướng này.
