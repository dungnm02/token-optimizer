# Gỡ bỏ Khung sườn Prompt (Rà soát Chi phí Tích lũy) (Tiếng Việt)

**Giải quyết:** Nguyên nhân 6.4 trong [`../CAUSE.md`](../CAUSE.md)

**Ý tưởng:** System prompt tích lũy khung sườn qua các thế hệ model — tường
...
## Tại sao khung sườn đắt gấp đôi

Khung sườn tốn kém hai lần: như **chi phí prompt cố định** trên mỗi
...
| Khung sườn | Tại sao nó từng tồn tại | Trạng thái hiện tại |
| --- | --- | --- |
...
| Các bộ few-shot dài | Hành vi zero-shot yếu | Thường có thể bỏ hoặc cắt còn 1 ví dụ trên các model hàng đầu |
...
Gỡ bỏ khung sườn là một chiến dịch xóa dựa trên đánh giá:
...
Mỗi thế hệ khiến một lớp khung sườn khác trở nên lỗi thời (hướng dẫn di
chuyển liệt kê rõ cái nào). Gấp một lượt loại bỏ khung sườn vào checklist
di chuyển — prompt được tinh chỉnh cho model cũ giờ vừa quá lớn vừa tinh
chỉnh sai.

### 5. Chuyển chỉ dẫn hiếm khi cần vào ngữ cảnh theo yêu cầu
...
nguyên (vài chục token) nằm trong ngữ cảnh cho đến khi nó được kích hoạt —
...
| --- | --- | --- |
| Hướng dẫn di chuyển của Anthropic / OpenAI | Tài liệu tham khảo | Danh sách chính thức về khung sườn nào mỗi thế hệ model làm lỗi thời |
...
- Đòi hỏi một bộ đánh giá thực sự — xóa mà không đo lường là cách các
  ...
  thư mục này.)
- Một số khung sườn chịu lực cho các trường hợp biên *của bạn* dù nhìn
