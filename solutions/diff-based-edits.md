...
**Ý tưởng:** Khi chỉnh sửa file — kiểu output phổ biến nhất của các coding
agent — chỉ nên để model phát ra **phần đã thay đổi (hunk)**: khối
search/replace, unified diff, hoặc lệnh gọi tool chỉnh sửa có mục tiêu.
Không bao giờ để model viết lại toàn bộ file.

---

## Tại sao điều này quan trọng

Viết lại một file 1.000 dòng chỉ để đổi 5 dòng tốn khoảng 1.000 dòng
output — loại token đắt nhất. Trong nhiều harness, file vừa viết lại đó
còn quay ngược vào context dưới dạng input ở lượt sau. Nhân chi phí này
với mọi lần chỉnh sửa trong một phiên, việc viết lại toàn bộ file thường
trở thành khoản chi phí *chi phối* của một coding agent. Cách làm này còn
chậm hơn (sinh nhiều token hơn nghĩa là độ trễ cao hơn) và rủi ro hơn (dễ
vô tình thay đổi cả những đoạn code không liên quan).
...
| --- | --- | --- |
| **Search/replace (str-replace)** | `old_string` chính xác → `new_string`, được kiểm tra tính duy nhất | Được dùng bởi tool `Edit` của Claude Code và tool text-editor của Anthropic (`str_replace_based_edit_tool`); độ tin cậy cao nhất vì harness xác minh anchor tồn tại chính xác một lần |
| **Unified diff** | Các đoạn `-`/`+` chuẩn | Phát hiện dựa trên benchmark của Aider: các định dạng kiểu diff giảm mạnh việc lược bỏ "lười biếng" (`# ...phần còn lại không đổi`) so với toàn bộ file trên các model frontier |
...
| **Toàn bộ file** | Nội dung đầy đủ | Chỉ dành cho file mới/nhỏ (dưới ~100 dòng thì chi phí viết lại vẫn thấp hơn rủi ro lỗi định dạng) |

### 2. Thực thi trong harness, không chỉ trong prompt

- Nên cung cấp các *tool chỉnh sửa* chuyên biệt (`str_replace`,
  `apply_diff`) thay vì một tool "write file" chung chung, để chỉnh sửa
  từng phần trở thành lựa chọn dễ dàng nhất.
- Xác thực anchor/đoạn ở phía harness (đảm bảo khớp đúng một lần, từ chối
  context đã cũ) và trả về lỗi rõ ràng, có thể hành động được — để một lần
  chỉnh sửa thất bại chỉ tốn một lượt thử lại nhỏ, thay vì phải quay về
  viết lại toàn bộ file.
- Theo dõi trạng thái file bằng hash tại lần đọc gần nhất, để harness có
  thể từ chối các chỉnh sửa dựa trên nội dung đã cũ, thay vì bắt model
  phải đọc lại rồi phát lại toàn bộ.

### 3. Dùng model fast-apply cho các lần gộp mờ (tùy chọn)

Một mẫu hình SOTA mới hơn: model frontier phát ra một *bản phác thảo*
chỉnh sửa lỏng lẻo, và một "model áp dụng" chuyên biệt nhỏ gộp nó vào file
với tốc độ hàng nghìn token/giây. Điều này giữ output của model frontier ở
mức tối thiểu trong khi vẫn chấp nhận các anchor không hoàn hảo.
...
| --- | --- | --- |
| Claude Code · Anthropic API | Tool `Edit` / `text_editor_20250728` | Search/replace được xác minh anchor; triển khai tham chiếu |
...
| Gemini CLI | Tool chỉnh sửa `replace` | Chỉnh sửa từng phần dựa trên anchor trong harness |
...
| Model fast-apply Morph / Relace | Thương mại | Gộp tốc độ cao chuyên biệt cho các chỉnh sửa lỏng lẻo (~hàng nghìn token/giây) đứng sau bất kỳ model frontier nào |

## Đánh đổi

- Định dạng chỉnh sửa đôi khi thất bại (không tìm thấy anchor, đoạn dị
  ...
  dạng.
- Search/replace cần anchor duy nhất; các file lặp lại nhiều buộc phải
  dùng chuỗi context lớn hơn.
...
- Lợi ích phụ về chất lượng: không xảy ra hồi quy ngoài ý muốn ở những
  vùng code chưa được động tới, và diff khi review sẽ khớp đúng với ý
  định thay đổi.
